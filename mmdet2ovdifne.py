...
def convert_keys(mmdet_weights):
    """根据映射规则转换key名称"""
    detr_weights = {}
    for old_key_, value in mmdet_weights.items():
        old_key = old_key_.replace('module.', '')
        ...
        # =====================different scale=====================
        # enc
        elif 'bbox_head.cls_branches.3' in old_key:
            new_key = old_key.replace('bbox_head.cls_branches.3', 'decoder.enc_score_head')
        ...
        print(f'convert {old_key_} to {new_key}')
    return detr_weights
...
if __name__ == "__main__":
    # 配置路径
    MMDET_WEIGHT_PATH = "../models/epoch_4.pth"  # 输入mmdet模型权重路径
    DETR_SAVE_PATH = "../models/epoch_4_convert.pth"  # 输出detr模型权重路径

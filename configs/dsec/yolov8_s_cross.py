...
# -----data related-----
data_root = '/repository/panshouan/dataset/DSEC/'  # Root path of data
# Path of train annotation file
train_ann_file = 'annotations/train_event.json'
train_data_prefix = 'train/'  # Prefix of train image path
# Path of val annotation file
val_ann_file = 'annotations/test_event.json'
...
base_lr = 0.01
max_epochs = 50  # Maximum training epochs TODO
...
model = dict(
    type='CMKDYOLODetector',
    teacher_config='configs/dsec/yolov8_s_rgb.py',
    teacher_ckpt='work_dirs/dsec/yolov8_s_rgb/best_coco_bbox_mAP_epoch_37.pth',
    no_share_params=[
        [0.25, 8, 16],
        [1, 4, 16],
        [4, 1, 16]
...
pre_transform = [
    dict(
        ...
        modal=('events_frame', 'images'), backend_args=None),
    dict(type='YOLOLoadAnnotations', with_bbox=True)
]

last_transform = [
    dict(
        type='MultiSpectralApply',
        transforms=dict(
            type='Albu',
    ...
    dict(type='MultiSpectralApply', transforms=dict(type='YOLOv5HSVRandomAug')),
    dict(type='RandomFlip', prob=0.5),
    dict(
        type='YOLOPackDetInputs',
...
test_pipeline = [
    ...
    dict(type='YOLOLoadAnnotations', with_bbox=True, _scope_='mmdet'),
    dict(
        type='PackDetInputs',

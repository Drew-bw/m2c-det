...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
            ...
            for _batch_input in _batch_inputs:
                ...
                if self._channel_conversion:
                    if self.multispectral and self.training:
                        _batch_input = _batch_input[[2, 1, 0, 5, 4, 3], ...]  # type: ignore
                    else:
                ...
                if self._enable_normalize:
                    ...
                    if self.multispectral and self.training:
                        ...
                        _batch_input = (_batch_input - self.mean[:3]) / self.std[:3]
        ...
        # Process data with `default_collate`.
            ...
            if self._channel_conversion:
                if self.multispectral and self.training:
                    _batch_inputs = _batch_inputs[:, [2, 1, 0, 5, 4, 3], ...]  # type: ignore
                else:
            ...
            if self._enable_normalize:
                if self.multispectral and self.training:
                    ...
                    _batch_inputs = (_batch_inputs - self.mean[:3]) / self.std[:3]
...
class BatchSyncRandomResize(nn.Module):
    ...
    def forward(
        ...
        if scale_x != 1 or scale_y != 1:
            ...
            # import cv2
            # img = inputs[0, 3:].permute(1, 2, 0).cpu().numpy() * 255
...
class BatchMixup(nn.Module):
    ...
    def forward(
        ...
        # Mixup data_samples
        if data_samples is not None:
            shifted_samples = copy.deepcopy(data_samples[-1:]) + copy.deepcopy(data_samples[:-1])
            mixup_data_samples = []
            for i in range(len(data_samples)):
                if 'text' in data_samples[i]:
                    num_class = len(data_samples[i].text)
                    sample_text = copy.deepcopy(data_samples[i].text)
                    shift_sample_text = copy.deepcopy(shifted_samples[i].text)
                    sample_gt = data_samples[i].gt_instances.labels.tolist()
                    shift_sample_gt = shifted_samples[i].gt_instances.labels.tolist()

                    # pos
                    pos_text = [sample_text[int(p)] for p in sample_gt]
                    shift_pos_text = [shift_sample_text[int(p)] for p in shift_sample_gt]
                    final_pos_text = list(set(pos_text + shift_pos_text))
                    pos_text2ids = {label: j for j, label in enumerate(final_pos_text)}

                    # neg
                    neg_text_num = num_class - len(final_pos_text)
                    if neg_text_num < 0:
                        inputs[i] = inputs_backup[i]
                        mixup_data_samples.append(copy.deepcopy(data_samples[i]))
                        continue
                    else:
                        neg_text = list(set(sample_text + shift_sample_text) - set(final_pos_text))
                        final_neg_text = random.choices(neg_text, k=neg_text_num)
                        final_text = final_pos_text + final_neg_text
                        for i1, cls in enumerate(sample_gt):
                            data_samples[i].gt_instances.labels[i1] = pos_text2ids[
                                sample_text[int(cls)]]
                        for i2, cls in enumerate(shift_sample_gt):
                            shifted_samples[i].gt_instances.labels[i2] = pos_text2ids[
                                shift_sample_text[int(cls)]]
                ...
                if 'text' in data_samples[i]:
                    if self.shuffle:
                        shuffle_id = np.random.permutation(len(final_text)).astype(np.int64)
                        new_final_text = ['0'] * len(final_text)
                        for old_id, new_id in enumerate(shuffle_id):
                            new_final_text[new_id] = final_text[old_id]
                        for ii, cls in enumerate(mixup_data_sample.gt_instances.labels):
                            mixup_data_sample.gt_instances.labels[ii] = shuffle_id[cls]
                    else:
                        new_final_text = final_text
                    mixup_data_sample.pop('text')
                    mixup_data_sample.set_data({'text': new_final_text})
    ...
    def vis(self, inputs, data_samples, index):
        import cv2
        img = inputs[index].transpose(0, 1).transpose(1, 2) * 255
        img = img.cpu().numpy()[..., ::-1].astype(np.uint8)
        img_ = img.copy()
        for ii, bboxes in enumerate(data_samples[index].gt_instances.bboxes):
            x1, y1, x2, y2 = bboxes
            cv2.rectangle(img_, (int(x1), int(y1)), (int(x2), int(y2)), (255, 0, 0), 2)
            labelid = int(data_samples[index].gt_instances.labels[ii])
            name = data_samples[index].text[labelid]
            cv2.putText(

...
def multi_num_points_multi_scale_deformable_attn_pytorch(
        ...
        num_points_list: List[int], deterministic=False) -> torch.Tensor:
    ...
    for level, (H_, W_) in enumerate(value_spatial_shapes):
        ...
        if deterministic:
            sampling_value_l_ = F.grid_sample(
                value_l_.half().double(),
                sampling_grid_l_.half().double(),
                mode="bilinear",
                padding_mode="zeros",
                align_corners=False
            ).to(value_l_.dtype)
...
class MultiNumPointsMultiScaleDeformableAttention(BaseModule):
    ...
    def forward(self,
        ...
        output = multi_num_points_multi_scale_deformable_attn_pytorch(
            ...
            self.num_points, deterministic=self.deterministic)
...
class MaxSigAttn(nn.Module):
    ...
    def __init__(
        ...
        self.query_proj_q = nn.Linear(embed_dims, embed_dims, bias=False)
        self.query_proj_k = nn.Linear(embed_dims, embed_dims, bias=False)
        self.text_proj_v = nn.Linear(embed_dims * 2, embed_dims, bias=False)
        self.bias = nn.Parameter(torch.zeros(num_heads))

    def forward(self, query, key):
        b, n, c = query.shape
        num_text = key.shape[1]

        query_feat_q = self.query_proj_q(query)
        query_feat_k = self.query_proj_k(query)
        text_feat_v = self.text_proj_v(key)

        query_feat_k = query_feat_k.reshape(b, n, self.num_heads, -1).permute(0, 2, 1, 3)
        text_feat_v = text_feat_v.reshape(b, num_text, self.num_heads, -1).permute(0, 2, 3, 1)
        text_attn = query_feat_k @ text_feat_v
        text_attn = text_attn.max(dim=-1)[0]
        text_attn = text_attn * self.scale + self.bias[None, :, None]
        text_attn = text_attn.sigmoid()

        query_feat_q = query_feat_q.reshape(b, n, self.num_heads, -1).permute(0, 2, 1, 3)
        query_feat_q = query_feat_q * text_attn.unsqueeze(-1)
        query_feat_q = query_feat_q.permute(0, 2, 1, 3).flatten(2)
        query = query + query_feat_q

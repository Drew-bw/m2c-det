...
class MultiSpectralLoadImageFromFile(LoadImageFromFile):

    def __init__(self,
                 modal: list[str, str] = ('infrared', 'visible'),
                 **kwargs) -> None:
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        results['img_path_sens'] = str(results['img_path']).replace(self.modal[0], self.modal[1])
        filename_sens = results['img_path_sens']
        try:
            if self.file_client_args is not None:
                file_client = fileio.FileClient.infer_client(
                    self.file_client_args, filename)
                file_client_sens = fileio.FileClient.infer_client(
                    self.file_client_args, filename_sens)
                img_bytes = file_client.get(filename)
                img_bytes_sens = file_client_sens.get(filename_sens)
            else:
                img_bytes = fileio.get(
                    filename, backend_args=self.backend_args)
                img_bytes_sens = fileio.get(
                    filename_sens, backend_args=self.backend_args)

            img = mmcv.imfrombytes(
                img_bytes, flag=self.color_type, backend=self.imdecode_backend)
            img_sens = mmcv.imfrombytes(
                img_bytes_sens, flag=self.color_type, backend=self.imdecode_backend)
        ...
        if self.to_float32:
            ...
            img_sens = img_sens.astype(np.float32)

        img = np.concatenate([img, img_sens], axis=2)
        results['img'] = img

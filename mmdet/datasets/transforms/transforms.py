...
class Mosaic(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'text' in results:
            mix_texts = sum([results["text"]] + [x["text"] for x in results["mix_results"]], [])
            mix_texts = list({tuple(x) for x in mix_texts})
            text2id = {text: i for i, text in enumerate(mix_texts)}

            for label in [results] + results["mix_results"]:
                for i, cls in enumerate(label['gt_bboxes_labels'].tolist()):
                    text = label["text"][int(cls)]
                    label['gt_bboxes_labels'][i] = text2id[tuple(text)]
                    label['instances'][i]['bbox_label'] = text2id[tuple(text)]
                label["text"] = mix_texts
...
class CachedMosaic(Mosaic):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'text' in results:
            mix_texts = sum([results["text"]] + [x["text"] for x in mix_results], [])
            mix_texts = list({tuple(x) for x in mix_texts})
            text2id = {text: i for i, text in enumerate(mix_texts)}

            for label in [results] + mix_results:
                for i, cls in enumerate(label['gt_bboxes_labels'].tolist()):
                    text = label["text"][int(cls)]
                    label['gt_bboxes_labels'][i] = text2id[tuple(text)]
                    label['instances'][i]['bbox_label'] = text2id[tuple(text)]
...
@TRANSFORMS.register_module()
class MultiSpectralApply(BaseTransform):
    annotations_list = [
        'instances', 'gt_bboxes', 'gt_bboxes_labels', 'homography_matrix',
        'gt_ignore_flags', 'gt_masks', 'gt_seg_map',
    ]
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        transforms.update(**kwargs)
        self.trans = TRANSFORMS.build(transforms)

    def pop_annotations(self, results: Dict):
        for anno in self.annotations_list:
            if results.get(anno, None) is not None:
                results.pop(anno)
        return results

    def transform(self, results: Dict) -> Optional[Dict]:
        """Randomly apply the transform."""
        results_sens = results.copy()
        # results_sens = self.pop_annotations(results_sens)

        # ====swap img===
        results_sens['img'] = results['img'][..., 3:]
        results['img'] = results['img'][..., :3]
        if len(results['img_shape']) == 3:
            results['img_shape'] = results['img'].shape
            results_sens['img_shape'] = results_sens['img'].shape

        # ====transforms===
        results = self.trans(results)
        results_sens = self.trans(results_sens)

        # =====merge======
        img = np.concatenate([results['img'], results_sens['img']], axis=-1)
        results['img'] = img
        if len(results['img_shape']) == 3:
    ...
    def __repr__(self) -> str:
        repr_str = self.__class__.__name__
        repr_str += f'(transforms = {self.trans}'
        return repr_str


from mmcv.transforms import Compose as mmcv_Compose
Transform = Union[Dict, Callable[[Dict], Dict]]

@TRANSFORMS.register_module()
class RandomMultiSpectralApply(BaseTransform):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.prob = prob
        self.transforms = mmcv_Compose(transforms)
    ...
    def transform(self, results: Dict) -> Optional[Dict]:
        ...
        if self.random_apply():
            results_sens = self.transforms(results_sens)
